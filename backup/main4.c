...
int main(int argc,char* argv[]){
	...
	int i=0;
	struct datacolumn*	dc1=NULL;
	struct datarow*		dr1=getrowfromindex(i,datacolumns);
	while(dr1){
		memset(querytext,0,1024);
		strcpy(querytext,"insert into test.test2 set");
		dc1=datacolumns;
		while((dr1=getrowfromindex(i,dc1))){
			sprintf(querytext,"%s %s='%s',",querytext,dc1->title,dr1->data);
			dc1=dc1->next;
		}
		...
		i++;
		dr1=getrowfromindex(i,datacolumns);
	}
